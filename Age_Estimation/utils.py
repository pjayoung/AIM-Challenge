...
def MAE(predict, age):
    ...
    predict_age = torch.sum(predict, dim=1)[:, 0] + 1
    abs_error = torch.sum(torch.abs(predict_age - age))
    mean_abs_error = abs_error / len(age)
    return mean_abs_error


def make_task_importance(label_file='/raid/co_show02/JY/CNN/txt/ca_rawlabels.txt'):
    # 나이 범주별로 샘플 수를 계산하기 위한 초기화
    category_counts = {'u20': 0, '20s': 0, '30s': 0, '40s': 0, 'over50': 0}

    # 라벨 파일을 읽어 나이 범주에 따라 샘플 수를 집계
    with open(label_file, 'r') as file:
        for line in file:
            _, age_category = line.strip().split('\t')
            if age_category in category_counts:
                category_counts[age_category] += 1

    # 샘플 수에 제곱근을 취해 중요도를 계산
    lambda_t = np.sqrt(list(category_counts.values()))
    summary = np.sum(lambda_t)
    lambda_t = lambda_t / summary

    return torch.tensor(lambda_t, dtype=torch.float32)


# def make_task_importance(data_path='/raid/co_show02/JY/Ordinal_Regression_with_Multiple_Output_CNN_for_Age_Estimation/AFAD-Full'):
#     lambda_t = []
...
def make_label(age):
    ...
    k = torch.tensor(torch.arange(1, 5).repeat(batch_size).reshape(batch_size, -1))
    label = k - age.reshape(batch_size, -1)
    label[label >= 0], label[label < 0] = 0, 1
    label = label.reshape(batch_size, 5 - 1, 1)
    true = torch.cat((label, 1 - label), dim=2)
    return true


def check_image(path='/raid/co_show02/JY/CNN/txt/ca_rawlabels.txt'):
